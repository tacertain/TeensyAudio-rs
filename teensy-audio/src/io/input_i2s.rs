...
//! ┌─────────┐        ┌───────────────────────┐              ┌──────────┐
//! │  RDR[0]  │──DMA─►│ 256 × u32 (128 frames)│──deinterl──►│ left  [0]│
//! │          │        │ L R L R L R ...       │────────────►│ right [1]│
//! └─────────┘        └───────────────────────┘              └──────────┘
//! ```
//!
...
//!
//! // In DMA RX ISR:
//! let should_update = input.isr(&DMA_RX_BUFFER);
...
///
/// The DMA engine fills a buffer with interleaved stereo data from SAI1 RX.
/// The ISR (via [`isr()`](Self::isr)) de-interleaves completed data into
/// separate left/right audio blocks. [`update()`](AudioNode::update) provides
/// the completed blocks as graph outputs and allocates fresh working blocks
/// for the next DMA cycle.
pub struct AudioInputI2S {
    ...
    /// Whether the working blocks are filled and ready for output.
    blocks_ready: bool,
...
impl AudioInputI2S {
    ...
    /// and splits the interleaved stereo data into the working left/right blocks.
    ///
    /// # Arguments
    ///
    /// - `dma_buffer`: The DMA receive buffer (`[u32; AUDIO_BLOCK_SAMPLES * 2]`).
    ///
    /// # Returns
    ///
...
mod tests {
    ...
    fn isr_fills_working_blocks() {
        ...
        // Create a DMA buffer with known data (2 words per frame, MSB-aligned)
    ...
    fn update_provides_filled_blocks() {
        ...
        // Fill with known pattern: left=100, right=200 (MSB-aligned)
        for i in 0..AUDIO_BLOCK_SAMPLES {
            dma_buf[i * 2] = (100u16 as u32) << 16;
            dma_buf[i * 2 + 1] = (200u16 as u32) << 16;
        }

        // One ISR call fills the blocks completely
        input.isr(&dma_buf);
    ...
    fn isr_signals_update_correctly() {
        ...
        assert!(input_responsible.isr(&dma_buf));
        assert!(!input_not.isr(&dma_buf));
    }
