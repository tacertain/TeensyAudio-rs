...
### PLL4 configuration values (from C++ reference)
```
PLL loop divider:  32
Numerator:         77
Denominator:       100
Post-divider:      /2 → MCLK = ~11.2896 MHz
BCLK divider:      8 → BCLK = ~1.4112 MHz
→ Fs = BCLK / 32 = ~44100 Hz
```
...
Build the forked teensy4-rs; write a minimal RTIC example that plays a 440Hz tone through SAI1 with DMA on the Teensy Audio Shield (SGTL5000), adapting from the existing `rtic_sai_sgtl5000.rs` but replacing interrupt-driven FIFO polling with DMA transfers.

### Success criteria
- Clean build on `thumbv7em-none-eabihf`
- Existing teensy4-rs examples still compile
- 440Hz tone audible on Audio Shield headphone output
- DMA transfers confirmed (no per-sample CPU intervention)
